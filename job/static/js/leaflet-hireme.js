...
window.map=L.map('map',mapOptions);$('#scrollEnabling').hide();function locationData(jobURL,companyLogo,companyName,jobTitle,verifiedBadge){return(''+
...
map.removeControl(map.zoomControl);var zoomOptions={zoomInText:'',zoomOutText:'',};var zoom=L.control.zoom(zoomOptions);zoom.addTo(map);}
function singleListingMap(){var lng=parseFloat($('#singleListingMap').data('longitude'));var lat=parseFloat($('#singleListingMap').data('latitude'));var singleMapIco="<i class='"+$('#singleListingMap').data('map-icon')+"'></i>";var hiremeicon=L.divIcon({iconAnchor:[0,0],popupAnchor:[0,0],className:'hireme-marker-icon',html:'<div class="marker-container no-marker-icon ">'+
'<div class="marker-card">'+
